...
import CYCLEGAN.transform_image as tfc
...
def set_tensorflow_generator(level):
    print('setting genererator')
    tf_model = tfc.LoadServedTF(level)
    global use_gan
    use_gan = True
    global cycleGAN
    cycleGAN = True
...
def make_env(rank, arguments, use_gan=False, which_epoch='', **kwargs):
    """
    creates environment
    ...
    def _thunk():
        ...
        if use_gan:
            if kwargs.get('CycleGAN', False):
                cycle_gan = set_tensorflow_generator(args.level)
                global tf_model
                tf_model = cycle_gan
            else:
...
def _process_frame84(frame):
    ...
    if use_gan:
        ...
        if cycleGAN:
            frame = tf_model.transform(frame)
        else:
            frame = model.transform(frame)

    # Here to switch to tensorflow
