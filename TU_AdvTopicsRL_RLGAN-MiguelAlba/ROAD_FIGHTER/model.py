...
from ROAD_FIGHTER.distributions import Categorical, DiagGaussian, Bernoulli
from ROAD_FIGHTER.utils_a2c import init, init_normc_
...
class Policy(nn.Module):
    ...
    def __init__(self, obs_shape, action_space, recurrent_policy):
        ...
        # choose the distribution by the action space type
    ...
    def act(self, inputs, states, masks, deterministic=False):
        ...
        dist_probs = torch.distributions.utils.logits_to_probs(dist.logits, True)

        if torch.cuda.is_available():
            action = action.cuda()
        action_log_probs = dist.log_probs(action)

        return value, action, action_log_probs, states, dist_probs
