...
def train(args, extra_args):
    ...
    alg_kwargs.update(extra_args.__dict__)
...
def run(play=False, **config):
    ...
    for key in config:
        if key in fallback:
            fallback.update({key: config[key]})
        if key in extra_fallback:
            extra_fallback.update({key: config[key]})
    if MPI is None or MPI.COMM_WORLD.Get_rank() == 0:
        rank = 0
        configure_logger(fallback.log_path)
    else:
        rank = MPI.COMM_WORLD.Get_rank()
        configure_logger(fallback.log_path, format_strs=[])

    model, env = train(fallback, extra_fallback)

    if fallback.save_path is not None and rank == 0:
        save_path = osp.expanduser(fallback.save_path)
