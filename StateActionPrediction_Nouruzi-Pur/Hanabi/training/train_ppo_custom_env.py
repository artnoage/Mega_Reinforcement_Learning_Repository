...
import custom_environment.predictive_models
...
# # # # # # # # # # # # # # # # # # # #
# For Predictive Model Implementation #
# # # # # # # # # # # # # # # # # # # #
saveModel = True
train = True
...
# ActionPred
actionPred = True
action_pred_l1 = 300            #neurons
action_pred_l2 = 170
action_pred_l3 = 170

# StatePred
statePred = False
state_pred_l1 = 170             #neurons
state_pred_l2 = 170 
curiosity_param = 0.01
curiosity = False
...
def train_eval(
    ...
    # ################################################ #
        ...
        state_pred_net= custom_environment.predictive_models.StatePredictor(state_pred_l1,state_pred_l2, num_parallel_environments, curiosity_param)
        action_pred_net = custom_environment.predictive_models.ActionPredictor(action_pred_l1,action_pred_l2,action_pred_l3)
            ...
            while sess.run(environment_steps_count) < num_environment_steps:
                ...
                if statePred or actionPred:
                    ...
                    if curiosity:                                                   #experimental
                        ...
                        collectedTrajectory=state_pred_net.augmentReward(collectedTrajectory)
                        ...
                        for j in range(len(collectedTrajectory[0][0])):
                            ...
                            sess.run(add_batch_op ,feed_dict={
                                                                step_type : collectedTrajectory[0][:,i],
                                                                state : collectedTrajectory[1].get("state")[:,i,:],
                                                                info : collectedTrajectory[1].get("info")[:,i],
                                                                mask : collectedTrajectory[1].get("mask")[:,i,:],
                                                                state2 : collectedTrajectory[1].get("state2")[:,i,:],
                                                                action : collectedTrajectory[2][:,i],
                                                                logits :  collectedTrajectory[3].get("logits")[:,i,:],
                                                                next_step_type : collectedTrajectory[4][:,i],
                                                                reward : collectedTrajectory[5][:,i],
                                                                discount : collectedTrajectory[6][:,i]
                                                            })
